...
{
    cout <<"~SerialNetTools()"<<endl;

    delete ros_thread_;
    delete mySerial_thread_;
    delete rosNode_;
    delete mySerial_;

    // ros_thread_ = nullptr;
    // mySerial_thread_ = nullptr;
    // rosNode_ = nullptr;
    // mySerial_ = nullptr;
}
...
int SerialNetTools::init()
{
     ...
     * rosNode 初始化
     */
    rosNode_ = new RosNode(argc_, argv_);
    rosNode_->init();
    ros_thread_ = new QThread();
    rosNode_->moveToThread(ros_thread_);
    // 如果QT线程启动，则通知ROS节点开始运行
    QObject::connect(ros_thread_, &QThread::started, rosNode_, &RosNode::run);
    // 启动ROS线程
    ros_thread_->start();
    // 如果ROS关闭，则通知QT界面程序退出
    QObject::connect(rosNode_, &RosNode::signal_rosShutdown, app_, &QCoreApplication::quit);
    // ROS节点的一些信号 接收到消息 signal_ros_strMsg
    QObject::connect(rosNode_, &RosNode::signal_ros_strMsg, mainWindow_, &MainWindow::slot_send_textEdit);
    // UI发送数据到ROS
    QObject::connect(mainWindow_, &MainWindow::signal_rosPub, rosNode_, &RosNode::slot_ros_pubStrMsg);
